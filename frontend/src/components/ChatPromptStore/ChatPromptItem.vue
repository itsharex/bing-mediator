...
import { NButton, NEllipsis, NTag, NThing, useMessage } from "naive-ui";
  ...
  <NThing class="hover:bg-gray-100 cursor-pointer p-5">
    <template #description>
      ...
      <div class="float-right">
        <NButton secondary type="info" size="small" @click="showEditPromptPop(source)">
          编辑
        </NButton>
        <NButton secondary class="ml-2" type="error" size="small" @click="delPrompt(source)">
          删除
        </NButton>
      </div>
    </template>
    <NEllipsis :tooltip="false" :line-clamp="2">
      {{ source.prompt }}
    </NEllipsis>
  </NThing>
</template>
