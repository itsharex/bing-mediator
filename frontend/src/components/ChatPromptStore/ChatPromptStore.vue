...
<template>
  <NModal v-model:show="isShowPromptSotre" class="w-11/12 xl:w-[900px]" preset="card" title="提示词库">
    <div class="flex justify-start flex-wrap gap-2 px-5 pb-2">
      ...
      <NButton secondary type="info" @click="showAddPromptPop">
        添加
      </NButton>
      <NUpload class="w-[56px] xl:w-auto" accept=".json" :default-upload="false" :show-file-list="false" @change="importPrompt">
        <NButton secondary type="success" :loading="isImporting">
          导入
        </NButton>
      </NUpload>
      <!-- <NButton secondary type="success">导入</NButton> -->
      <NButton secondary type="success" :loading="isExporting" @click="exportPrompt">
        导出
      </NButton>
      <NButton secondary type="error" @click="clearPrompt">
        清空
      </NButton>
    </div>
    <VirtualList
  ...
  <NModal v-model:show="optPromptConfig.isShow" class="w-11/12 xl:w-[600px]" preset="card" :title="optPromptConfig.title">
    <NSpace vertical>
      标题
      <NInput v-model:value="optPromptConfig.newPrompt.act" placeholder="请输入标题" />
      描述
      <NInput v-model:value="optPromptConfig.newPrompt.prompt" placeholder="请输入描述" type="textarea" />
      <NButton block secondary type="info" @click="savePrompt">
        保存
      </NButton>
    </NSpace>
  </NModal>
  <NModal v-model:show="isShowDownloadPop" class="w-11/12 xl:w-[600px]" preset="card" title="下载提示词">
    <NList class="overflow-y-auto rounded-lg" hoverable clickable>
      <NListItem v-for="(config, index) in promptDownloadConfig" :key="index">
        ...
        <NInput v-else-if="config.type === 2" v-model:value="config.url" placeholder="请输入下载链接，支持 json 及 csv " />
        <template #suffix>
          <div class="flex justify-center gap-5">
            <a v-if="config.type === 1" class="no-underline" :href="config.refer" target="_blank" rel="noopener noreferrer">
              <NButton secondary>来源</NButton>
            </a>
            <NButton secondary type="info" :loading="config.isDownloading" @click="downloadPrompt(config)">
              下载
            </NButton>
          </div>
        </template>
