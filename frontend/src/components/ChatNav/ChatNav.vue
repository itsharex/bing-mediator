...
import { type DropdownOption, NButton, NDropdown, NImage, NInput, NModal, useMessage } from "naive-ui";
...
const navConfigs = [
  {
    key: navType.github,
    label: "开源地址",
    url: "https://github.com/kirklin/bing-mediator",
  },
  {
    key: navType.version,
    label: "版本信息",
...
<template>
  <NDropdown v-if="isMobile()" class="select-none" :show="isShowMore" :options="navConfigs" :render-label="renderDropdownLabel" @select="handleSelect">
    <NImage class="fixed top-6 right-4 cursor-pointer z-50" :src="settingSvgUrl" alt="设置菜单" :preview-disabled="true" @click="isShowMore = !isShowMore" />
  </NDropdown>
  <NDropdown v-else class="select-none" trigger="hover" :options="navConfigs" :render-label="renderDropdownLabel" @select="handleSelect">
    <NImage class="fixed top-6 right-6 cursor-pointer z-50" :src="settingSvgUrl" alt="设置菜单" :preview-disabled="true" />
  </NDropdown>
  <NModal v-model:show="isShowSetTokenModal" preset="dialog" :show-icon="false">
    ...
    <NInput v-model:value="userToken" size="large" type="text" placeholder="用户 Cookie ,仅需要 _U 的值" />
    <template #action>
      <NButton size="large" @click="isShowSetTokenModal = false">
        取消
      </NButton>
