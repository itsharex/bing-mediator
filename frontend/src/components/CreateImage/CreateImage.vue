...
<template>
  <NModal v-model:show="isShowModal" class="w-11/12 lg:w-[540px] select-none" :on-close="onClose" preset="card" title="图像创建">
    <head class="flex justify-center gap-3 px-8">
      <NInput v-model:value="keyword" class="flex-1" placeholder="提示词" :clearable="true" maxlength="100" @keyup.enter="createImage" />
      <NButton secondary type="info" :loading="isCreating" @click="createImage">
        开始创建
      </NButton>
    </head>
    <main class="flex justify-center items-center">
      <iframe v-if="iframeSrc" class="w-[310px] h-[350px] xl:w-[475px] xl:h-[520px] my-4" :src="iframeSrc" frameborder="0" @load="isCreating = false" />
      <NEmpty v-else class="h-[40vh] xl:h-[60vh] flex justify-center items-center" description="暂未创建">
        <template #extra>
          <NButton secondary type="info" @click="useDemo">
            使用示例创建
          </NButton>
