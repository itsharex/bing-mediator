...
import { NButton, NEmpty, NInput, NResult, useMessage } from "naive-ui";
import { storeToRefs } from "pinia";
import VirtualList from "vue3-virtual-scroll-list";
import ChatPromptItem from "./ChatPromptItem.vue";
import { type IPrompt, usePromptStore } from "@/stores/modules/prompt";
  ...
  <footer>
    ...
    <div v-if="isShowUnauthorizedModal" class="fixed top-0 left-0 w-screen h-screen flex justify-center items-center bg-black/40 z-50">
      <NResult class="box-border w-11/12 lg:w-[400px] px-4 py-4 bg-white rounded-md" status="403" title="401 未授权">
        <template #footer>
          <NInput v-model:value="authKey" class="w-11/12" type="password" placeholder="请输入授权码" maxlength="60" clearable />
          <NButton class="mt-4" secondary type="info" :loading="isAuthBtnLoading" @click="auth">
            授权
          </NButton>
        </template>
      </NResult>
